...
// Events whose [start, end) ranges intersect a candidate range, other
// than (optionally) itself. Used to warn before creating a double-booked
// event, not to prevent it outright — overlaps are allowed if chosen.
export function findOverlaps(events, start, end, excludeId) {
  return events.filter((e) => e.id !== excludeId && e.start < end && e.end > start);
}
...
// A task with no due date belongs only to the actual current day, not
// to every day (or every future day someone happens to be previewing)
// at once. It still isn't lost if it doesn't get done — since "today"
// keeps advancing, it keeps showing up each real day until it's
// completed, same rolling-forward behavior as a task with a real due
// date (see CLAUDE.md's shame-free principle) — it just never shows up
// on a future day being merely previewed before that day arrives.
export function scheduleTasks(tasks, dayEvents, forDate, dayBounds) {
  ...
  // Someday tasks sit out of the daily plan entirely — never scheduled,
  // never "rolling to another day," never counted as overflow. They only
  // ever move by someone deliberately promoting one back to active.
  const eligible = tasks
    .filter((t) => !t.someday)
    .filter((t) => (t.dueDate ? t.dueDate <= forKey : forKey === todayKey));
...
// For Focus mode: the one task to show instead of the full list. Prefers
// whichever undone task has a real scheduled slot soonest; only falls
// back to priority when nothing undone has a time at all (i.e. every
// undone task overflowed today's plan).
export function pickNextTask(scheduledTasks) {
  const undone = scheduledTasks.filter((t) => !t.done);
  if (undone.length === 0) return null;
  const timed = undone.filter((t) => !t.overflow && t.scheduledStart != null);
  if (timed.length > 0) {
    return timed.reduce((soonest, t) => (t.scheduledStart < soonest.scheduledStart ? t : soonest));
  }
  return undone.reduce((best, t) => (t.priority < best.priority ? t : best));
}
