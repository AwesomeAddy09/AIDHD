...
export async function insertTasks(supabase, userId, tasks) {
  const rows = tasks.map((t) => ({
    ...
    steps: null,
    // A someday task has no real timeline by definition — never carries
    // a due date over, regardless of what else was parsed for it.
    due_date: t.someday ? null : t.date || null,
    someday: !!t.someday,
  }));
...
function rowToProfile(row) {
  ...
  return {
    ...
    recorderConsentAckAt: row.recorder_consent_ack_at,
    ...
    completionSoundEnabled: row.completion_sound_enabled,
    taskDensity: row.task_density,
    plan: row.plan,
...
// Lesson Recorder: only the transcript and summary are ever persisted
// (never the raw audio), and only until the user taps Discard.
...
export async function fetchRecordings(supabase) {
  const { data, error } = await supabase
    .from("recordings")
    .select("*")
...
export async function insertRecording(supabase, userId, { transcript, summary }) {
  const { data, error } = await supabase
    .from("recordings")
    .insert({ user_id: userId, transcript, summary })
    .select()
    .single();
  if (error) throw error;
...
export async function deleteRecording(supabase, id) {
